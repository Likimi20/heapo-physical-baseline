...
> # ⚠ FROZEN AND STALE — 2026-09-12. DO NOT BUILD AGAINST THIS FILE.
>
> **The live contract is [`SCHEMA_v2.md`](SCHEMA_v2.md)** (`physical_baseline_v2`,
> 214 × 65). This v1 document describes a superseded 47-column export and is kept
> on disk **for one cycle only**, per its own side-by-side rule below.
>
> What changed, so nothing here is read as current:
> * **v1 predates the sourced constants registry, the station normal-year weather
>   and the heat-pump vintage term.** Its numbers are not comparable with v2's.
> * **v1's fault columns no longer exist.** Aaditya's fault prediction was removed
>   from this workstream (Miguel, 2026-09-12); v2 asserts on write that no column
>   name contains "fault". Anything below about a fault axis is history.
> * **`BELOW_PHYSICS` was renamed `USES_LESS_THAN_MODELLED`** — "below" reads as
>   "good".
> * v1's statuses and counts are superseded; v2 ships four states with
>   HIGH_EXCESS 40 · EXCESS_CONFIRMED 118 · INCONCLUSIVE 14 · INCOMPLETE_AUDIT 42.
...
setting-fault axis, now populated from Aaditya's model as a suggestion (see *Visit
axis*).~~ **STALE 2026-09-12: that axis was removed from this workstream. See the
banner at the top of this file and `SCHEMA_v2.md`.**
